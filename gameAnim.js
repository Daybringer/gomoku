...
function init() {
  ...
  const sectionDimensions = document
  ...
  const secondaryColor = hexToRgbA(
    document.getElementById("colorSecond").innerHTML
  );
  ...
  GLOBAL_INTERVAL = setInterval(() => {
    if (gamePlan.state === "done") {
      gamePlan.indexOfCurr = pickGamePlan(
        gameSet.length,
        gamePlan.indexesOfPlayed
      );
      if (gamePlan.indexOfCurr === -1) {
        resetCanvas();
      } else {
        const position = generetePosition(
          gameSet[gamePlan.indexOfCurr],
          gamePlan.playedPos
        );
        if (position === -1) {
          resetCanvas();
        } else {
          gamePlan.playedPos.push([
            position[0],
            position[1],
            gameSet[gamePlan.indexOfCurr].width,
            gameSet[gamePlan.indexOfCurr].height,
          ]);
  ...
  function generetePosition(gameObj, oldPos) {
    for (let x = 0; x < 100; x++) {
      ...
      if (checkIfPosAvailable(xRand, yRand, oldPos, gameObj))
        return [xRand, yRand];
    }
    return -1;
  }
  ...
  function checkIfPosAvailable(posX, posY, oldPos, gameObj) {
    for (let x = 0; x < oldPos.length; x++) {
      if (
        !(
          posX + gameObj.width < oldPos[x][0] ||
          posX > oldPos[x][0] + oldPos[x][2] ||
          posY + gameObj.height < oldPos[x][1] ||
          posY > oldPos[x][1] + oldPos[x][3]
        )
      ) {
        return false;
      }
  ...
  function placeLine() {
    ...
    const { x: fromX, y: fromY, radius } = calcPosition(
      fromXGrid + gamePlan.position[0],
      fromYGrid + gamePlan.position[1]
    );
    const { x: toX, y: toY } = calcPosition(
      toXGrid + gamePlan.position[0],
      toYGrid + gamePlan.position[1]
    );
