...
function createPlayerProbabilitiesFn(
  ...
  const expectedPlayerProbability: ExpectedProbabilitiesFn = (
    playerARating,
    playerBRating,
  ) => {
...
function createNextRatingFn(kFactor: number | KFactorFunction): NextRatingFn {
  const kFactorFn: KFactorFunction =
    typeof kFactor === 'number' ? () => kFactor : kFactor;
  ...
  return (
    ...
    const change = Math.round(
      kFactorFn(rating) * (actualPoints - expectedPoints),
    );
 ...
 */
export type KFactorFunctionWithPlayers = (
  rating: number,
...
export function createRatingSystem(
  ...
  const symmetricKFactorFn = (rating: number) =>
    resolvedKFactor(rating, undefined);
  const getPlayerProbabilities = createPlayerProbabilitiesFn(
    exponentBase,
    exponentDenominator,
  );
  ...
  const getNextRatings = createNextRatingsFn(
